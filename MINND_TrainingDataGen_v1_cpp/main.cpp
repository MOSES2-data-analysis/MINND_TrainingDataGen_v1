...
int main(int argc, char** argv) {
    ...
    /* Allocate memory */
    int truth_cube[SPATIAL_DIM][SPECTRAL_DIM][CHANNELS] = {0}; // randomly generated truth in x - lambda space
    int plus[outboard_sz];
    int zero[x_dim]; // Results of MOSES forward model
    int minus[outboard_sz];
    int input_cube[SPATIAL_DIM][SPECTRAL_DIM][CHANNELS] = {0}; // Combination of results to feed into bottom of CNN
    ...
    for (int count = 0; count < DATASET_SZ; count++) {
        ...
        truth_cube[x][y][PLUS] = I;
        truth_cube[x][y][ZERO] = I;
        truth_cube[x][y][MINUS] = I;
